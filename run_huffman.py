...
for id1 in train_id:
    ...
    for _, row in user_record.iterrows():
        ...
        if len(rating) % 32 is 0:
            ...
            loss = agent.learn(state_list, np.array(state_length_list), np.array(action_list), Q_value)
...
for id1 in test_id:
    ...
    for _, row in user_record.iterrows():
        ...
        if len(all_state) > 1:
            ...
            output_action = agent.get_all_action_prob(temp_state, temp_state_length).flatten()
